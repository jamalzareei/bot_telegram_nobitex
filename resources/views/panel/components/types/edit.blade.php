...
<form action="{{ route('panel.types.update.type') }}" method="post" class="ajaxForm">
    @csrf
                        ...
                        <select name="model_type" class="form-control select2" id="model_type">
                            ...
                            @isset($models)
                                @forelse ($models as $model)
                                    <option value="{{ $model }}" {{ $type && $model == $type->model_type ? 'selected' : '' }}> {{ $model }}</option>
                                @empty
                                @endforelse
                            @endisset
                        </select>

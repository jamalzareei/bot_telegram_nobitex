...
<section id="data-list-view" class="data-list-view-header">
            ...
            <tbody>
                @forelse ($statuses as $status)
                    <tr>
                        ...
                        <td>{{ $status->name }}</td>
                        <td>{{ $status->slug }}</td>
                        <td>{{ $status->model_type }}</td>
                        <td>
                            <div class="custom-control custom-switch custom-switch-success mr-2 mb-1">
                                <input type="checkbox" name="active_at" class="custom-control-input" id="active_at" {{ $status->actived_at ? 'checked' : '' }} >
                        ...
                        <td><a href="{{ route('panel.statuses.list', ['model'=>$status->model_type]) }}"><i class="fa fa-eye"></i></a></td>
                        <td class="product-action">
                            <span class="action-edit" onclick="editRow('{{ route('panel.statuses.edit.status', ['id'=> $status->id]) }}')"><i class="feather icon-edit"></i></span>
    ...
    <div class="add-new-data-sidebar">
        <div class="overlay-bg"></div>
            ...
            @include('panel.components.statuses.add',[
